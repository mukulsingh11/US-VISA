...
class DataTransformation:
    ...
    def get_data_transformation_object(self) -> ColumnTransformer:
        try:
            ...
            ordinal_pipeline = Pipeline(stepe=[
                ('imputer',SimpleImputer(strategy='most_frequent')),
            ...
            transform_pipelien = Pipeline(steps=[
                ('scaler',StandardScaler()),
                ('transformer',PowerTransformer())
            ])

            # Columns Transformer enables us to apply transform to particular columns.
            # it help us ti fit multiple transformations to multiple columns with a single fit() or fit_transform() statements.

            preprocessor = ColumnTransformer([
                ('num_pipeline' , num_pipeline,numerical_columns),
                ('onehot_pipeline',onehot_pipeline,onehot_columns),
                ('ordinal_pipeline',ordinal_pipeline,ordinal_columns),
                ('power_transformer', transform_pipelien,transform_columns)
            ])
            return preprocessor
        except Exception as e:
            raise CustomException(e,sys) from e
        ...
        def _remove_outliers_IQR(self,col ,df):
            try:
                ...
                df.loc[(df[col]<lower-limit), col] = lower_limit 
                return df               
        ...
        def initiate_data_transformation(self) -> DataTransformationArtifact:
            try:
                ...
                smt = SMOTEENN(random_state=40,sampling_strategy='all')
                ...
                logging.info(f"saving transformed and test array")
