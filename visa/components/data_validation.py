...
class DataValidation:
    def __init__(self, data_validation_config: DataValidationConfig,
        ...
        try:
            logging.info(f"{'>>' * 20 } Data Validatio log started {'<<' * 20 }")
        ...
        try:
            ...
            if self.isFolderPathAvailable() == True:
                train_filename = os.path.basename(
                    self.data_ingestion_artifact.tarin_file_path)
                
                is_train_filename_validated = self.train_data.validte_filename(file_name = train_filename)

                is_train_column_numbers_validated = self.train_data_validate_column_length()
